...
@desc: Use a ball tree to find vectors maximising inner product with a query efficiently.
       Based on P. Ram & A. Gray, Maximum Inner-Product Search using Tree Data-Structures, 2012.
@appreciation: Some scripts are heavily borrowed from https://github.com/gamboviol/miptree
'''
import random
import numpy as np
...
class MIPTree(object):
    ...
    def build(self, S, node):
        if len(node.S) > self.N0:
            ...
            if len(node.S[dd <= 0]) != 0 and len(node.S[dd>0]) != 0:
